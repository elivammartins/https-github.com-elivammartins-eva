...
import { TravelInfo, MediaApp, TrackMetadata, MediaViewState, AppSettings, CarStatus, StopInfo, RouteWarning } from './types';
...
const toolDeclarations: FunctionDeclaration[] = [
  {
    ...
    parameters: {
      type: Type.OBJECT,
      description: 'Telefone/WhatsApp. Respeite o Modo Ghost.',
      properties: {
  ...
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      description: 'Navegação em players de vídeo por série, temporada e episódio.',
      properties: {
        appId: { type: Type.STRING, enum: ['youtube', 'netflix', 'disney', 'prime'] },
  ...
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: { 
        // Fix: Updated enum to include all possible CarAction values used in the UI
...
const App: React.FC = () => {
  ...
  const [travel, setTravel] = useState<TravelInfo>({ 
    destination: 'SEM DESTINO', stops: [], warnings: [
      ...
      { id: '2', type: 'FLOOD', distance: 1200, description: 'Ponto Alagado (Histórico)', coords: [-23.560, -46.640] }
    ], drivingTimeMinutes: 45, totalDistanceKm: 12, weatherStatus: 'CÉU LIMPO', floodRisk: 'LOW'
  });
  ...
  const handleSystemAction = async (fc: any) => {
    ...
    if (fc.name === 'car_control') {
      // Fix: Actually update car status in local state when commanded
      setCarStatus(prev => {
      ...
      return { result: "COMANDO ENVIADO AO VEÍCULO." };
    }
    return { result: "OK" };
  };
  ...
  const startVoiceSession = async () => {
    ...
    try {
      // Fix: Guideline requires checking for selected API key for high-quality models if Veo is used later
      ...
      // Fix: Creating fresh instance before use
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          // Fix: Removed googleSearch tool as it conflicts with function calling declarations in SDK guidelines
          tools: [{ functionDeclarations: toolDeclarations }],
          systemInstruction: `SISTEMA PANDORA EVA V160.
          1. PROATIVIDADE: Você tem liberdade para intervir. Avise sobre: Alagamentos (floodRisk), Clima (weatherStatus), Radares próximos ou notícias relevantes.
          2. VÍDEO: Sem trava de velocidade. Se o usuário quiser vídeos ou séries, use media_navigation com appId, série, temporada e episódio.
          3. NAVEGAÇÃO: Gerencie paradas. Se ele pedir para ir a um lugar ou remover parada, faça-o.
          4. VEÍCULO: Você controla o carro Elivam Martins via car_control.
          Seja a assistente inteligente e prestativa que Elivam merece.`
        }
      });
    ...
    return (
      <div className="h-screen w-screen bg-black flex flex-col items-center justify-center p-10 italic text-white">
         <div className="w-56 h-56 rounded-full border-4 border-blue-500/20 p-2 mb-12 animate-glow-blue flex items-center justify-center shadow-2xl">
            <div className="w-full h-full rounded-full bg-blue-600/10 flex items-center justify-center border-2 border-blue-500/50"><i className="fas fa-car-side text-6xl text-blue-400"></i></div>
         </div>
         <h1 className="text-3xl font-black mb-8 uppercase tracking-tighter">PANDORA CORE V160</h1>
         <button onClick={startVoiceSession} className="h-20 px-12 bg-blue-600 rounded-[35px] font-black uppercase shadow-[0_0_50px_rgba(37,99,235,0.4)] hover:bg-blue-500 transition-all active:scale-95">Link Eva Core</button>
      </div>
    );
  }
    ...
    <div className="h-screen w-screen bg-black text-white flex overflow-hidden font-sans italic">
      ...
      {isCollisionRisk && (
        <div className="fixed inset-0 z-[5000] border-[20px] border-red-600 animate-pulse pointer-events-none flex items-center justify-center bg-red-600/10">
           <div className="bg-red-600 px-16 py-8 rounded-full font-black text-4xl shadow-2xl uppercase italic text-white border-4 border-white/20">Impacto Iminente!</div>
        </div>
      )}
      ...
      <aside className={`h-full z-20 bg-[#0a0a0c] border-r border-white/5 flex flex-col p-6 transition-all duration-700 ${mapFullScreen ? 'w-0 -ml-10 opacity-0' : 'w-[40%]'}`}>
         <header className="flex items-center justify-between mb-8 mt-4">
            <div className="flex flex-col">
               ...
               <div className="flex items-center gap-3">
                  ...
                  <div className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 text-[8px] font-black rounded border border-emerald-500/20">OSRM ACTIVE</div>
               </div>
            </div>
         ...
         <footer className="h-28 pt-4 border-t border-white/5 shrink-0">
            <MiniPlayer app={currentApp} metadata={track} onControl={(a) => handleSystemAction({name: 'media_navigation', args: {appId: currentApp.id, action: a}})} onExpand={() => setMediaState('FULL')} transparent />
         </footer>
      </aside>
      ...
      <main className="flex-1 relative bg-zinc-900">
         <MapView 
           ...
           onRouteUpdate={(steps, d, dist) => setTravel(p => ({...p, drivingTimeMinutes: d, totalDistanceKm: dist}))}
         />
         
         {/* PIP VIDEO */}
         {mediaState === 'PIP' && (
           <div className="absolute top-10 right-10 w-[350px] h-[200px] z-[200] animate-fade-in shadow-2xl">
              <EntertainmentHub speed={currentSpeed} currentApp={currentApp} track={track} isPip onMaximize={() => setMediaState('FULL')} onClose={() => setMediaState('HIDDEN')} onControl={(a) => handleSystemAction({name: 'media_navigation', args: {appId: currentApp.id, action: a}})} />
           </div>
         )}
         ...
         {mediaState === 'FULL' && (
           <div className="absolute inset-0 z-[1000] bg-black animate-fade-in">
              <EntertainmentHub speed={currentSpeed} currentApp={currentApp} track={track} onMinimize={() => setMediaState('PIP')} onClose={() => setMediaState('HIDDEN')} onControl={(a) => handleSystemAction({name: 'media_navigation', args: {appId: currentApp.id, action: a}})} />
           </div>
         )}
      </main>
      ...
      <AddStopModal 
        ...
        onAdd={(n, la, ln) => {
          const newStop: StopInfo = { id: Date.now().toString(), name: n, type: 'REST', coords: [la, ln], timeFromPrev: '12m', distanceFromPrev: '3.4km' };
